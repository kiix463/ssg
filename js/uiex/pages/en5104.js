...
const _templates = {
    screenSaver: 'Enable screen saver({0})',
}
...
const tcrDraw = () => {
    ...
    if (menu) {
        if (Array.isArray(menu.textvalues)) {
            ...
            const screenSaver = menu.textvalues.find(t => t.option_id == 'textvalue3')

            $('#idHour').val((hour && hour.option_value) || '')
            $('#idMin').val((minute && minute.option_value) || '')
            $('#idScreenSaver').html(_templates.screenSaver.format((screenSaver && screenSaver.option_value) || ''))
        }
